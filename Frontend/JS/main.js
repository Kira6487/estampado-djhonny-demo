(() => {
  ...
  const adminBase = `${publicBase}/admin`;
  ...
  if (header && !isAdmin) {
    header.innerHTML = `
      <div class="topbar"><div class="container topbar__inner"><span><i class="bi bi-truck me-1"></i>Envíos a todo el Perú</span><span><i class="bi bi-whatsapp me-1"></i>WhatsApp sujeto a validación</span></div></div>
  ...
  if (footer && !isAdmin) {
    footer.innerHTML = `<footer class="site-footer"><div class="container"><div class="row g-4 align-items-center"><div class="col-md-3"><img class="footer-logo" src="${assetBase}/djhonny-logo.jpg" alt="Estampado D'Jhonny"></div><div class="col-md-5"><h2 class="h6 text-uppercase">Base del catálogo digital</h2><p class="mb-0 text-white-50">Precios referenciales por lote sujetos a validación comercial.</p></div><div class="col-md-4"><h2 class="h6 text-uppercase">Navegación</h2><a class="me-3" href="${publicBase}/catalogo.html">Catálogo</a><a href="${publicBase}/contacto.html">Contacto</a></div></div><hr class="footer-divider"><p class="mb-0 small text-white-50">© <span data-current-year></span> Estampado D'Jhonny · Fase 1 académica.</p></div></footer>`;
  }
